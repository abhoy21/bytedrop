...
export const Navbar: React.FC = () => {
  ...
  useEffect(() => {
    setRandomIndex(Math.floor(Math.random() * Profileimg.length));
  }, []);

  return (
    <>
      <nav className='absolute top-0 left-0 right-0 px-4 py-4 flex justify-between items-center bg-transparent'>
        <Link
          href='#'
          className='flex items-center justify-center space-x-2 text-3xl font-bold leading-none'
        >
          <CloudRain className='h-10 w-10 text-purple-600' />
          <p className='font-mono text-purple-600'>ByteDrop</p>
        </Link>

        <ul className='hidden lg:flex lg:mx-auto lg:items-center lg:w-auto lg:space-x-6'>
          <li>
            <Link
              className='text-sm text-purple-500 hover:text-blue-500 font-bold'
              href='#'
            >
              Home
            </Link>
        ...
        <div className='h-10 w-10 md:h-12 md:w-12 overflow-hidden rounded-full ring-2 ring-gray-700 dark:ring-gray-100'>
          {randomIndex !== null && (
            <Image
              width='50'
              height='50'
